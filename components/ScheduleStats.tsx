...
const ScheduleStats: React.FC = () => {
  ...
  const stats = useMemo(() => {
    const data: Record<string, Record<string, { total: number, courses: Record<string, number> }>> = {}; 
      ...
      teachers.forEach(t => {
          ...
          if (tStats) {
              Object.entries(tStats).forEach(([month, data]) => {
                  Object.entries(data.courses).forEach(([courseId, hours]) => {
                                        ...
                                        {Object.entries(entry.courses).map(([cId, hrs]) => {
                                            ...
                                            return (
                                                <div key={cId} className="flex justify-between items-center text-xs text-slate-600 bg-slate-100 p-1.5 rounded">
                                                    ...
                                                    <span className="font-bold">{hrs.toFixed(1)}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
